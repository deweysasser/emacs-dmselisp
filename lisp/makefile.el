;;; Emacs 19.19 comes with this file (or one like it)
...
for completions on macro names."
  ...
  (let ((macro-name))
    (if n
	(progn (setq macro-name (get-macro-name))
	       (end-of-line 1)
	       (insert "\n" macro-name "="))
      (insert "=")
      (save-excursion
	(re-search-backward "^\\s-*\\(\\w*\\)\\s-*=" nil 't)
	(setq macro-name (buffer-substring (match-beginning 1)
					   (match-end 1)))))
...
(defun get-macro-name ()
    (interactive )
    (read-from-minibuffer "Macro Name:"))
